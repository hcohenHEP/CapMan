...
{
    int i;
    image im = (image)malloc(sizeof(image_t) + h * sizeof(rgb_t*)+ sizeof(rgb_t) * w * h);
    im->w = w; im->h = h;
    im->pix = (rgb_t**)(im + 1);
...
void PreMedianFilterImage(int size, const char* inFile, const char* outFile)
{
    image in, out;
    if (size < 0) size = 1;
